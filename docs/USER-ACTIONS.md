...
- [ ] Как модерировать: в карточке жми **«✅ Опубликовать»** → откроется чат с ботом →
      жми **«Отправить»** (там уже вписано `/ok N`). Обработка до ~5 минут.
      «❌ Отклонить» — так же (`/no N`). Или пиши `ok N` / `no N` руками.
- [ ] НЕ запускай `npm run tg:bot` локально — конфликтует с Actions (двойная обработка)
