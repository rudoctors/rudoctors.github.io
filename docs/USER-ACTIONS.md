...
## F. Telegram-модерация «Добавить врача» — **готово, работает без твоих действий** (24.09.2026)

Заявки из формы приходят как GitHub Issues (label `doctor-request`). Бот
**@rudoctors_moderation_bot** присылает их в Telegram (аккаунт Iva / balkandunav).

- [x] Бот создан через @BotFather от аккаунта `new`; секреты в Actions (`gh secret set`)
- [x] **Хостинг — GitHub Actions**: уведомления мгновенно, ничего держать запущенным не нужно
- [x] E2E: «✅» публикует карточку (деплой сам), «❌» закрывает заявку — проверено вживую
- [x] **Форма теперь отправляет в Telegram напрямую (25.09):** открывается чат бота
      с уже вписанной заявкой — посетитель жмёт «Отправить», всё. Бот сам создаёт
      заявку и показывает вам карточку; посетителю уходит «Заявка принята».
- [x] E2E: intake от «посетителя» → issue → карточка → reject — проверено вживую
- [ ] Как модерировать: в карточке жми **«✅ Опубликовать»** → откроется чат с ботом →
      жми **«Отправить»** (там уже вписано `/ok N`). Обработка — при очередном запуске
      воркфлоу (мгновенно при пуше/локальном боте; cron GitHub бывает задержан — тогда
      Actions → Telegram moderation → Run workflow). «❌» — так же (`/no N`).
